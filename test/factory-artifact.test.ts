...
function createJob(snapshot: Record<string, unknown>): VaultPluginFactoryJob {
  return {
    pluginName: "test-plugin",
    snapshot
  } as VaultPluginFactoryJob;
}

describe("Factory artifact evidence", () => {
  it("accepts a persisted verified artifact for real Vault approval", () => {
    const job = createJob({
  ...
  it("restores artifact evidence from the completed auto-repair result", () => {
    const job = createJob({
      generated: { files },
      autoRepair: { status: "pass", artifact, scaffoldSha256 }
    });
  ...
  it("rejects an unstored or checksum-mismatched artifact", () => {
    const unstored = createJob({
      artifactSha256: sha256,
      generated: { buildArtifact: { sha256 }, files },
      autoRepair: { status: "pass", artifact: { sha256 }, scaffoldSha256 }
    });
    const mismatched = createJob({
  ...
  it("rejects a verified artifact when the saved source differs from the built source", () => {
    const job = createJob({
      artifactSha256: sha256,
